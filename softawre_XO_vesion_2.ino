...
void loop() {
  ...
  else if  (end_game==false and niveau==2) {
    uti_jouer() ;
    affichage_plateau() ;
  ...
  else if (end_game==false and niveau==1) { 
    uti_jouer() ;
    affichage_plateau() ;
    if (gagne(symbole_uti)==true) { 
      delay(500) ;
...
void robot_jouer_1() { 
  if(one_symbole_in_grille()==true) {
    if(ligne_first_symbole_uti!=1 and col_first_symbole_uti!=1) {
      ligne_robot=1 ;
      col_robot=1 ;
    }
    else {
